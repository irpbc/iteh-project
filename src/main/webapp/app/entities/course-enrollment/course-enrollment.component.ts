...
@Component({
    selector: 'jhi-course-enrollment',
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.courseEnrollmentService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/course-enrollment'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: CourseEnrollment) {
        return item.id;
    }
    registerChangeInCourseEnrollments() {
        this.eventSubscriber = this.eventManager.subscribe('courseEnrollmentListModification', (response) => this.loadAll());
    }
